// Clean app script: local quotes + images manifest
(function(){
  const CATEGORIES = ["мотивация","успех","здоровье","отношения","карьера","саморазвитие","спорт","бизнес"];
  const LS = { prefs:"moti_prefs_v1", viewed:"moti_viewed_v1", fav:"moti_favorites_v1", pop:"moti_popularity_v1", cache:"moti_cache_v1", seen:"moti_seen_ids_v1", images:"moti_images_v1" };
  const LOCAL_QUOTES_URL = './quotes.json';
  const PRELOAD = 5, MAX_CACHE=200, LIKE_W=3, DISLIKE_W=-1;
  ...
  const state = {
    ...
    favs: load(LS.fav, []),
    pop: load(LS.pop, {}),
    seen: new Set(load(LS.seen, [])),
    cache: load(LS.cache, []),
    images: load(LS.images, []),
    imagesByCat: {}
  };
  ...
  async function ensureImages(min=100){ const m=await loadManifest(); if(!m) return; const by={}, flat=[]; for(const k in m){ const mapped=mapCat(k); const arr=m[k]||[]; if(!by[mapped]) by[mapped]=[]; for(const u of arr){ by[mapped].push(u); flat.push(u); } } state.imagesByCat=by; state.images=flat; save(LS.images, flat); }
  function pickImage(cat){ const c=mapCat(cat); const arr=state.imagesByCat[c]; if(arr?.length) return arr[Math.floor(Math.random()*arr.length)]; if(state.images?.length) return state.images[Math.floor(Math.random()*state.images.length)]; return '' }

  async function getLocal(n=20){ try{ const r=await fetch(LOCAL_QUOTES_URL,{cache:'no-store'}); if(!r.ok) throw 0; const data=await r.json(); const out=[], used=new Set(); while(out.length<n && used.size<data.length){ const i=Math.floor(Math.random()*data.length); if(used.has(i)) continue; used.add(i); out.push(norm(data[i])); } return out; }catch(_){ return [] } }

  function rankAndQueue(cands){ const seen=new Set(); const unique=[]; for(const q of cands){ if(seen.has(q.id) || state.seen.has(q.id)) continue; seen.add(q.id); unique.push(q) } unique.sort(()=>Math.random()-.5); for(const q of unique){ if(state.queue.length>=PRELOAD+1) break; state.queue.push(q); state.seen.add(q.id); } save(LS.seen,[...state.seen]); }
  function showLoading(v){ $('#loading').classList.toggle('hidden',!v) }
  async function ensurePreload(){ if(state.queue.length>=PRELOAD) return; showLoading(true); try{ let batch=[]; if(state.cache?.length){ batch = state.cache.slice(0, PRELOAD*2).map(norm) } if(batch.length<PRELOAD){ const loc = await getLocal(PRELOAD*2); batch = batch.concat(loc); const newCache=[...batch.map(q=>({...q,isNew:false})), ...(state.cache||[])]; state.cache = Array.from(new Map(newCache.map(x=>[x.id,x])).values()).slice(0,MAX_CACHE); save(LS.cache,state.cache); } rankAndQueue(batch); } finally { showLoading(false) } }
  ...
  function renderNextCard(){ if(!state.queue.length){ ensurePreload().then(()=>{ if(state.queue.length) renderNextCard() }); return } const q=state.queue.shift(); state.current=q; const el=cardElement(q); stack.innerHTML=''; stack.appendChild(el); state.readingStart=now(); updateStats(); ensurePreload(); }

  $("#btnLike").addEventListener('click',()=>{ const card=$('.card'); if(!card||!state.current) return; finalizeSwipe('right',card,state.current) });
  $("#btnNope").addEventListener('click',()=>{ const card=$('.card'); if(!card||!state.current) return; finalizeSwipe('left',card,state.current) });
  ...
  $("#openFav").addEventListener('click', openFavPanel);
